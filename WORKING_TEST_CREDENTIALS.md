# 🔐 Wyshkit Platform - Working Test Credentials

**Date:** October 21, 2025  
**Status:** All Platforms Tested & Working  
**Server:** `http://localhost:8080` (Note: Server runs on port 8080, not 8081)
...
- **Authentication:** Guest browsing + Google OAuth + Email signup
...
#### Option 1: Guest Browsing (No Login Required)
```
URL: http://localhost:8080
Features: Browse products, add to cart, checkout as guest
```
...
Test Account:
- Email: customer-test@wyshkit.com
- Password: Customer123!
```

#### Option 3: Google OAuth Login
```
...
#### Option 1: Use Created Test Partner Account
```
URL: http://localhost:8080/partner/login
Email: partner-test@wyshkit.com
Password: Partner123!
Business: Test Gift Shop
Status: Account created, needs email verification
```

#### Option 2: Create New Partner Account
```
URL: http://localhost:8080/partner/signup
Process: Complete signup form
...
#### Current Status: Needs Database Setup
```
URL: http://localhost:8080/admin/login
Status: Login form works, but no admin accounts exist in database
```
...
2. **Create Admin User (if needed):**
   ...
   VALUES ('admin@wyshkit.com', crypt('admin123', gen_salt('bf')), NOW(), NOW(), NOW());
   
   INSERT INTO admin_users (id, email, role, created_at)
   VALUES (auth.uid(), 'admin@wyshkit.com', 'admin', NOW());
   ```
...
3. **Test Email/Password Login:**
   - Use credentials: `partner-test@wyshkit.com` / `Partner123!`
   - Click "Sign In"
...
- Run SQL migration to create admin user
...
### Admin Panel ✅
- [ ] Login page loads
- [ ] Admin credentials work (after setup)
